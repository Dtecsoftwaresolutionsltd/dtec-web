...
@section('title')
    <title>{{ $seo_setting?->seo_title ?? 'Products' }}</title>
    <meta name="title" content="{{ $seo_setting?->seo_title ?? 'Products' }}">
    <meta name="description" content="{!! strip_tags(clean($seo_setting?->seo_description ?? '')) !!}">
@endsection
@section('frontend_content')
    <main>
        <!-- breadcrumb -->
        <x-breadcrumb name="{{ __('Products') }}" />
                ...
                <div class="grid grid-cols-6 lg:grid-cols-12 box-border gap-[30px]">
                    <div class="col-span-5 flex justify-center flex-col">
                        <h1 class="text-24 md:text-48 font-semibold text-main-black mt-2.5 md:mt-5">
                            {{ $digital_transforming_brands ? getTranslatedValue($digital_transforming_brands, 'title') : '' }}
                        </h1>
                        <p class="mt-5 md:mt-10 text-paragraph pb-10">
                            {{ $digital_transforming_brands ? getTranslatedValue($digital_transforming_brands, 'short_description') : '' }}
                        </p>
                    ...
                    <div class="col-span-7 relative flex flex-col sm:flex-row justify-end w-full overflow-hidden">
                        @if($digital_transforming_brands)
                        ...
                        <div
                            ...
                            <h1 class="text-18 text-white font-semibold tracking-tight max-w-[170px]"
                                ...
                                data-speed-qs="1000">
                                {{ $digital_transforming_brands ? getTranslatedValue($digital_transforming_brands, 'award_winning_text') : '' }}
                            </h1>
                        </div>
                    </div>
                </div>
        ...
        <!-- about end  -->
        <section id="product" class="w-full">
            <div
                ...
                <div class="w-full product-section-wrapper relative">
                    <div class="theme-container mx-auto relative z-10">
                        <div class="flex flex-col items-center mb-10 md:mb-[70px]">
                            <h1
                                ...
                                {{ $product_explore_products ? getTranslatedValue($product_explore_products, 'section_name') : '' }}
                            </h1>
                            <h2
                                class="sm:text-48 text-24 font-semibold text-main-black text-center lg:w-[685px] w-full">
                                {{ $product_explore_products ? getTranslatedValue($product_explore_products, 'title') : '' }}
                            </h2>
                        </div>
                        <div class="w-full grid xl:grid-cols-12 md:grid-cols-6 grid-cols-3 xl:gap-[30px] gap-5">
                            @foreach ($product_explores ?? [] as $key => $product)
                                <div data-aos="zoom-out" data-aos-delay="{{ $key == 0 ? 0 : $key * 200 }}"
                                    ...
                                    <div
                                        ...
                                        <div
                                            ...
                                            <span>
                                                <img src="{{ asset($product?->theme_5_thumbnail_image) }}" alt="">
                                            </span>
                                        </div>
                                        <div class="mt-5 mb-4">
                    ...
                    <div
                        ...
                        <div class="max-w-[850px] w-full flex justify-center items-center flex-col relative z-10">
                            <h1
                                ...
                                {{ $product_faqs ? getTranslatedValue($product_faqs, 'section_name') : '' }}
                            </h1>
                            <h2 class="text-24 sm:text-48 font-semibold text-main-black mt-5">
                                {{ $product_faqs ? getTranslatedValue($product_faqs, 'title') : '' }}
                            </h2>
